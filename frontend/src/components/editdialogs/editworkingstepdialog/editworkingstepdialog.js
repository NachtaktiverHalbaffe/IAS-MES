...
Short description: Dialog to editing a specific working step
...
import {AUTO_HIDE_DURATION, DEFINED_TASKS, DEFINED_PLC_STATES} from "../../../../src/const";
...
export default function EditStateWorkingStepDialog(props) {
    ...
    // if dialog cards should be set to an empty state after leaving dialog
    ...
    const handleSave = () => { // validate data
        ...
        if ((state["task"] === "unstore" || state["task"] === "store") && state["assignedToUnit"] != 1) {
            setErrorState({snackbarOpen: true, msg: "Task store and unstore must be assigned to resource 1", level: "error"});
            return false;
        }
        if ((state["task"] === "package" || state["task"] === "unpackage" || state["task"] === "color" || state["task"] === "assemble" || state["task"] === "generic") && state["assignedToUnit"] == 1) {
            setErrorState({snackbarOpen: true, msg: "Resource 1 can only execute tasks store and unstore", level: "error"});
            return false;
        }
        // validate stepNo. It has to be a number
        if (isNaN(state["stepNo"]) || state["stepNo"] < 1) {
            setErrorState({snackbarOpen: true, msg: "Step number should be a number greater than 0. Its a convention to choose numbers multiple to 10 e.g. 20", level: "error"});
            return false;
        }
        ...
        if (!/^#[0-9A-F]{6}$/i.test(state["color"])) {
            ...
            setErrorState({snackbarOpen: true, msg: "Color is not formatted as a hex color. Format: #0dccff", level: "error"});
            return false;
        }
        // validate if name is shorter than 30 characters
        if (state["name"].length > 30) {
            setErrorState({snackbarOpen: true, msg: "Name is too long. Max length: 30", level: "error"});
            return false;
        }
        ...
        if (state["description"] > 200) {
            setErrorState({snackbarOpen: true, msg: "Description is too long.Max length: 200", level: "error"});
            return false;
        }
        if (isNaN(state["assignedToUnit"]) || state["assignedToUnit"] < 1 || state["assignedToUnit"] > 6) {
            setErrorState({snackbarOpen: true, msg: "Invalid resourceId of assigned unit. Value must be between 1-6", level: "error"});
            return false;
        }

        if (onSave(state)) {
            if (state["stepNo"] % 10 !== 0) {
                setErrorState({snackbarOpen: true, msg: "Step number should be a multiple of 10", level: "warning"});
            } else {
                setErrorState({snackbarOpen: true, msg: "Sucessfully added workingstep", level: "success"});
            }
    ...
    // update state of dialog when color gets changed from colorpicker
            ...
            <Dialog onClose={handleClose}
                ...
                <Grid container direction="column" justify="center" alignItems="center">
                    <EditTextBox label="Name" mapKey="name"
                        ...
                        onEdit={onEdit}/>
                    <EditChoiceBox label="Task" mapKey="task"
                        initialValue={
                            data["task"]
                        }
                    ...
                    <EditTextBox label="Assigned resource" mapKey="assignedToUnit"
                        initialValue={
                            data["assignedToUnit"]
                        }
